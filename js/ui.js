...
export function renderBlocks(blocks, cbs) {
  ...
  for (const b of blocks) {
    ...
    card.className='block-card'+(b.applied?' applied':'')+(b.balloonId?' balloon':'');
    card.dataset.id=b.id;
    const confColor=b.confidence>70?'#2d9e5f':b.confidence>40?'#e07d10':'#c0392b';
    const balloonBadge=b.balloonType?`<span class="block-balloon-badge">${_balloonIcon(b.balloonType)}</span>`:'';
    card.innerHTML=`
      <div class="block-header">
        <span class="block-num">#${b.id.split('-')[1]??b.id}</span>
        ${balloonBadge}
    ...
    card.querySelector('.block-translation').addEventListener('input',(e)=>cbs.onTranslationEdit(b.id,e.target.value));
...
function _balloonIcon(type) {
  return type==='thought'?'💭':type==='narration'?'📋':'💬';
}
function _esc(s) {
