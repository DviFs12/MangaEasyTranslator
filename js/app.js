 ...
 *  - text-layer recebe pointer-events:auto por padrão e perde quando
 *    ferramenta de desenho está ativa (brush/eraser/clone/lasso/stroke/select-rect)
 *  - renderer.onToolChange ajusta pointer-events do text-layer
 *  - textRend recebe getScale callback correto
 *  - setSize chamado após loadImage
 *  - Tradução individual por bloco via onTranslate
 *  - _applyBlock usa _renderToCtx (método correto)
 *  - Export compõe base + preview numa canvas temporária
 *  - Todos os botões do painel de configuração funcionam
 */
...
import {
  ...
  restoreProjectData, startAutosave, loadAutosave,
...
const textRend = new TextRenderer(textLayerEl, previewCanvas, () => renderer.scale);
...
// ── Tool change → ajusta pointer-events do text-layer ─────────────────────
// Ferramentas de desenho bloqueiam interação com caixas de texto
const DRAWING_TOOLS = new Set(['brush', 'eraser', 'clone', 'lasso', 'stroke']);

renderer.onToolChange = (tool) => {
  // Quando ferramenta de seleção/rect: text-layer recebe eventos (pode clicar nas caixas)
  // Quando ferramenta de desenho: text-layer ignora eventos (canvas recebe o mouse)
  textLayerEl.style.pointerEvents = DRAWING_TOOLS.has(tool) ? 'none' : 'auto';
};
...
store.subscribe((state, action) => {
  ...
  if (BLOCK_MUTATIONS.has(action.type)) {
    blockPanel.render(state.blocks, state.activeBlockId);
    // Sincronizar caixas DOM com o estado
    const ids = new Set(state.blocks.map(b => b.id));
    for (const id of [...textRend._boxes.keys()]) {
      if (!ids.has(id)) textRend.remove(id);
    }
  ...
  if (action.type === 'SELECT_BLOCK') {
    ...
    if (block) {
      boxEditor.show(block);
      textRend.select(block.id);
    }
    _activateTab('blocks');
  }

  if (action.type === 'DESELECT_BLOCK') {
    boxEditor.hide();
    textRend.deselect();
  }
...
async function _loadImageFile(file) {
  ...
  try {
    ...
    // Dimensionar text-layer e preview ao tamanho do canvas
    textRend.setSize(baseCanvas.width, baseCanvas.height);

    dispatch({
      type: 'LOAD_IMAGE',
      payload: { image: img, name: file.name.replace(/\.[^.]+$/, '') },
    });
...
async function _ocrBlock(id) {
  ...
  try {
    ...
    if (result) {
      dispatch({ type: 'UPDATE_BLOCK', payload: { id, text: result.text, confidence: result.confidence } });
    }
  } catch (e) {
    toast(t(store.getState().i18n, 'toastError', e.message), 'error');
  } finally {
  ...
  try {
    ...
    const trs   = await translateBatch(texts, toLang, srcLang, {
      onProgress: (done, total) =>
        updateLoading(Math.round(done / total * 100), `${done}/${total}…`),
    });
    dispatch({
      type: 'SET_ALL_TRANSLATIONS',
      payload: pending.map((b, i) => ({ id: b.id, translation: trs[i] })),
    });
...
function _applyBlock(id) {
  ...
  const { x, y, w, h } = block.bbox;
  // Capturar região original antes de renderizar
  const snapshot = renderer.getRegionSnapshot(x, y, w, h);
  // Renderizar a tradução diretamente no canvas base
  const ctx = baseCanvas.getContext('2d');
  textRend._renderToCtx(ctx, block);

  dispatch({
    type: 'MARK_APPLIED',
    payload: { id, snapshotData: _toBase64(snapshot) },
  });
  toast('Aplicado ✓');
}
...
function _unapplyBlock(id) {
  ...
  const img = new Image();
  img.onload = () => {
    baseCanvas.getContext('2d').drawImage(img, 0, 0, w, h, x, y, w, h);
  };
...
function _undo() {
  if (store.canUndoCanvas()) {
    const snap = store.popCanvasUndo();
    if (snap) renderer.restoreSnapshot(snap);
  }
  store.undoBlock();
}
...
function _redo() {
  if (store.canRedoCanvas()) {
    const snap = store.popCanvasRedo();
    if (snap) renderer.restoreSnapshot(snap);
  }
  store.redoBlock();
}
...
function _export() {
  ...
  textRend.renderToCanvas(ctx);  // caixas não-aplicadas
...
renderer.onSelectionChange = (sel, tool) => {
  ...
  // Posicionar abaixo da seleção
  const sr = stage.getBoundingClientRect();
  const cx = sr.left + renderer.tx + (sel.rect.x + sel.rect.w / 2) * renderer.scale;
  const cy = sr.top  + renderer.ty + (sel.rect.y + sel.rect.h)     * renderer.scale + 10;
  selToolbar.style.left = Math.min(window.innerWidth  - 300, Math.max(4,  cx - 140)) + 'px';
  selToolbar.style.top  = Math.min(window.innerHeight - 52,  Math.max(60, cy))        + 'px';
};
...
// Eventos vindos do HTML (atalhos de teclado definidos em index.html)
...
// Toolbar de seleção
$('btn-sel-ocr')?.addEventListener('click',     () => { if (_currentSel) _ocrSelection(_currentSel); });
$('btn-sel-clean')?.addEventListener('click',   () => { if (_currentSel) _cleanSelection(_currentSel); });
$('btn-sel-inpaint')?.addEventListener('click', () => { if (_currentSel) _inpaintSelection(_currentSel); });
$('btn-sel-text')?.addEventListener('click',    () => { if (_currentSel) _addTextBox(_currentSel); });
$('btn-sel-clear')?.addEventListener('click',   () => renderer.clearSelection());
...
// ── Bootstrap ─────────────────────────────────────────────────────────────
_applyI18n();
// Inicializar pointer-events do text-layer para 'auto' (ferramenta padrão = select)
